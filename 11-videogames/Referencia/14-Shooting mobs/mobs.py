...
class Mob (pygame.sprite.Sprite):
    def __init__(self, game, name, x, y, groups, image, speed, health, weapon_name):
    ...
    def shoot(self, x, y, bullet_group):
...
class Player (Mob):
    def __init__(self, game, x, y, groups, image, speed, health, weapon_name):
        """HO! HO! HO! Now I have a SHOTGUN"""
        super().__init__(game, 'PLAYER', x, y, groups, image, speed, health, weapon_name)
    ...
    def handle_input(self):
        ...
        if mouse[0]:
            ...
            self.shoot(x, y, self.game.player_bullets)
...
class SpittingBee (Mob):
    def __init__(self, game, name, x, y, groups, image, speed, health, weapon_name):
        """Adding some variance to base stats"""
        speed = random.uniform(speed * 0.9, speed * 1.1)
        health = random.uniform(health * 0.9, health * 1.1)
        print("Spitting bee born")
        super().__init__(game, name, x, y, groups, image, speed, health, weapon_name)

    def update(self):
        """Update avoid vector! and some crude patrolling"""
        towards_player = self.game.player.pos - self.pos
        required_distance = random.uniform(150, 200)

        shoot_point = self.game.player.pos - \
            towards_player.normalize() * required_distance
        towards_target = shoot_point - self.pos

        self.avoid_mobs()
        if 0 < towards_target.magnitude() < MOBS[self.name]['VISION_RADIUS']:
            self.vel = self.vel * 0.75 + towards_target.normalize() * 0.25
            if random.random() < MOBS[self.name]['SHOOT_P']:
                x, y = self.game.player.pos
                self.shoot(x, y, self.game.enemy_bullets)
        else:
            if random.uniform(1, 100) < 10:
                self.vel = vec(random.uniform(-1, 1), random.uniform(-1, 1))
        self.move()
...
class BeeNest (Nest):
    def __init__(self, game, name, x, y, groups, image, health, spawn_frequency, max_population):
        super().__init__(game, name, x, y, groups, image,
                         health, spawn_frequency, max_population)
...
class SpittingBeeNest (Nest):
    def __init__(self, game, name, x, y, groups, image, health, spawn_frequency, max_population):
        super().__init__(game, name, x, y, groups, image,
                         health, spawn_frequency, max_population)
        print("A spiting bee nest")

    def spawn_creature(self):
        offset = self.pos + vec(random.uniform(-25, 25),
                                random.uniform(-25, 25))
        SpittingBee(self.game, 'SPITTING_BEE', offset.x, offset.y, (self.game.all_sprites, self.game.mobs, self.spawn_group),
                    self.game.images['SPITTING_BEE'], MOBS['SPITTING_BEE']['SPEED'], MOBS['SPITTING_BEE']['HEALTH'], 'GUN')
