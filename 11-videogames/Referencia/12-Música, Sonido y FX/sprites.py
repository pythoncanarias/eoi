...
class Mob (pygame.sprite.Sprite):
    ...
    def hit(self, damage):
        """Play a random FX from the hit fx array"""
        self.health -= damage
        if self.health < 0:
            random.choice(self.game.critter_death_fx).play()
            self.kill()
...
class Player (Mob):
    ...
    def handle_input(self):
        """Play random gun FX when shooting"""
        self.vel = vec(0, 0)
        ...
        if mouse[0] and pygame.time.get_ticks() - self.last_shot_time > BASIC_GUN_FIRING_RATE:
            ...
            random.choice(self.game.basic_gun_fx).play()
...
class Item (pygame.sprite.Sprite):
    def __init__(self, game, x, y, type):
        ...
        self.pick_fx = game.item_fx[type]
