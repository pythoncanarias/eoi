...
class Map:
    ...
    def create_sprites_from_data(self, game):
        """Lets create a nest that spawns spitting bees"""
        for row in range(0, self.map_height):
            for col in range(0, self.map_width):
                ...
                if tile == '1':
                    ...
                    Tile(game, 'REGULAR_WALL', x, y, is_wall_top)
                if tile == 'P':
                ...
                if tile == 'B':
                    ...
                    BeeNest(game, nest_name, x, y, (game.all_sprites,
                                                    game.nests),
                            game.images[nest_name],
                            nest['HEALTH'],
                            nest['SPAWN_FREQUENCY'],
                            nest['MAX_POPULATION'])
                if tile == 'b':
                    print(f"spitting bee nest at {x},{y}")
                    nest_name = 'SPITTING_BEENEST'
                    nest = NESTS[nest_name]
                    SpittingBeeNest(game, nest_name, x, y, (game.all_sprites,
                                                            game.nests),
                                    game.images[nest_name],
                                    nest['HEALTH'],
                                    nest['SPAWN_FREQUENCY'],
