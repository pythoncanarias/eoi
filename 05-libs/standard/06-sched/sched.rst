...
a intervalos determinados. El módulo de la librería estándar **sched**
proporciona esta funcionalidad.

Los sistemas operativos proporcionan en muchos casos esta misma funcionalidad,
por ejemplo cron_ en Linux o `Task scheduler`_ en *Windows*, la ventaja de usar
el módulo de Python es que nos aisla de las diferencias en las plataformas, y
que nos permite incluir estas tareas programadas como parte de nuestro
programa.
...
Lo más sencillo es programar una tarea para que se ejecute
después de un lapso de tiempo:

.. literalinclude:: ejemplo01.py
    :language: python
    :lines: 4-
...
4. Se ejecuta el método ``run()`` del controlador ``scheduler``, con el
   parámetro ``blocking`` a verdadero. En este punto estamos cediendo el
   control del programa al ``scheculer``, que ejecutara las tareas que tenga
   programadas y que seguirá haciéndolo hasta que pulsemos la combinación
   ``Ctrl-C``.

.. note:: **Ejercicio**: Modificar el ejemplo anterior para que se ejecute cada
   cinco segundos, en vez de cada 10.
...
podría tener el valor por defecto de ``time.time()``. Lo mismo al usar el
método ``enter``, que nos obliga a definir un valor de prioridad que en
principio ni vamos a usar, y que podría tener perfectamente un valor por
omisión. A pesar de todo, este módulo puede ser interesante siempre que
tengamos procesos que se ejecuten de forma periódica o siguiente un esquema
...
en el momento que queremos, pero esto es tedioso y aburrido, así que la librería
proporciona un método específico para estos casos, el método ``enterabs``, en
el que simplemente indicamos el momento exacto en que queremos que se dispare
nuestra acción. En el siguiente ejemplo se programa una tarea para que se
ejecute según el esquema previsto, a las 09:00 y a las 18:30:

.. literalinclude:: ejemplo02.py
    :language: python
    :lines: 4-
...
4. En base a la fecha actual, creamos instancias de ``datetime`` para las ocho
   de la mañana y programamos el *backup* para esa hora.

5. Igual que (4), pero a las cuatro y media de la tarde.

6. Arrancamos el ``scheduler`` con el método ``run``, para que gestione él las
   tareas programadas.

.. note:: **Ejercicio** Modificar el ejemplo para que se ejecute uno y dos
   minutos a partir de la hora actual, para comprobar que el programa está
...
Se puede conseguir más información en la `documentación oficial de sched`_.

.. _cron: https://es.wikipedia.org/wiki/Cron_(Unix)
.. _documentación oficial de sched: https://docs.python.org/3/library/sched.html
.. _Task scheduler: https://es.wikipedia.org/wiki/Planificador_de_tareas_(Windows)
