...
- La clase `datetime.datetime` es una combinación de fecha y hora, con
  los atributos: `year`, `month`, `day`, `hour`, `minute`, `second`,
  `microsecond` y `tzinfo`

- La clase `datetime.timedelta` representa una duración: La diferencia
...
Los objetos de tipo `time` se puede comparar entre si, si sin del mismo tipo
...
Otras formas de crear objetos de tipo `time` son similares a las vistas para
las clase de tipo `date`: `fromisoformat` crea una instancia de `time` a partir
de una cadena de texto, por ejemplo `time.fromisoformat('04:23:01')` o
`time.fromisoformat('04:23:01.000384')` o incluso
`time.fromisoformat('04:23:01+04:00')`, que nos crea una objeto de tipo `time`
_aware_, porque le hemos indicado la diferencia con UTC, 4 horas.
Igualmente, tiene el método inverso `isoformat`.

El método `replace` funciona de forma similar al mismo método de la case
`date`.

## Objetos de tipo marca tamporal o `datetime`
...
formato ISO, con `fromisoformat`, aunque para _parsear_ cadenas de texto existen
otras librerías de terceros como `dateutil`, `arrow` y otras, más potentes.
